...
A delegation tool's input schema SHALL be derived from the target's `manual` trigger: one
required parameter per name in `requires`, described as a locked variable, and nothing else —
no prose argument, no fan-out argument. The tool description SHALL be built from the target's
own spec (title, requires, returns), never its `instructions`. A dispatch missing a required
name SHALL be refused with kind `missing-param` before any child is composed; an argument
carrying a `${{…}}` reference SHALL be resolved against the caller's variables verbatim (not
glob-escaped) before dispatch, and an unresolvable reference SHALL refuse the call with kind
`unresolved-param`.
...
declares `returns` — the values of those variables read from the child's settled store — and
